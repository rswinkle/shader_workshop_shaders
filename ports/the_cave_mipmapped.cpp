...
//
// ---------------------------------------------------------------------------
// the_cave_mipmapped.cpp — same demo as the_cave.cpp, but written for static
// 2D mip chains + explicit screen-space gradients (texture2DGrad).
//
// Keep the_cave.cpp as the pre-mip baseline.
//
// mainImage: straightforward port (host texture2DGrad).
// mainFrame: inlined sw_texture2DGrad, camera/light hoisted once per frame.
// ---------------------------------------------------------------------------
...
{
	pgl_vec4 s = texture2DLod(ch, uv.x, uv.y, lod);
	return vec4(s.x, s.y, s.z, s.w);
}
...
{
	assert(t);
	pgl_vec4 s = sw_texture2DLod(t, uv.x, uv.y, lod);
	return vec4(s.x, s.y, s.z, s.w);
}

static inline vec4 texGrad(GLuint ch, vec2 uv, vec2 dUVdx, vec2 dUVdy)
{
	pgl_vec4 s = texture2DGrad(ch, uv.x, uv.y,
	                           dUVdx.x, dUVdx.y,
	                           dUVdy.x, dUVdy.y);
	return vec4(s.x, s.y, s.z, s.w);
}
...
{
	assert(t);
	pgl_vec4 s = sw_texture2DGrad(t, uv.x, uv.y,
	                             dUVdx.x, dUVdx.y,
	                             dUVdy.x, dUVdy.y);
	return vec4(s.x, s.y, s.z, s.w);
}

static inline vec3 dn_from_dv(vec3 n, float vlen, vec3 dv)
{
	return (dv - n * dot(n, dv)) / vlen;
}

static vec3 nmap(vec2 t, GLuint tx, float str, vec2 dUVdx, vec2 dUVdy)
{
	float d=1.0f/1024.0f;
	float xy = texGrad(tx, t, dUVdx, dUVdy).x;
	float x2 = texGrad(tx, t + vec2(d, 0.f), dUVdx, dUVdy).x;
	float y2 = texGrad(tx, t + vec2(0.f, d), dUVdx, dUVdy).x;
...
static vec3 nmap(vec2 t, const glTexture* tx, float str, vec2 dUVdx, vec2 dUVdy)
{
	float d=1.0f/1024.0f;
	float xy = texGrad(tx, t, dUVdx, dUVdy).x;
	float x2 = texGrad(tx, t + vec2(d, 0.f), dUVdx, dUVdy).x;
	...
	const glTexture* ch0, const glTexture* ch1, const glTexture* ch2)
{
	vec3 Pos = view.oPos;
	vec3 oPos = view.oPos;
	vec2 uv = 2.f * fragCoord / view.r2 - 1.f;
	vec3 v = vec3(uv * vec2(view.aspect, 1.0f), 1.0f);
	float vlen = length(v);
	vec3 vn = v / vlen;
	vec3 Dir = vn * view.cam;

	vec3 dv_dx = vec3(view.aspect * 2.f / view.r2.x, 0.f, 0.f);
	vec3 dv_dy = vec3(0.f, 2.f / view.r2.y, 0.f);
	vec3 dDir_dx = dn_from_dv(vn, vlen, dv_dx) * view.cam;
	vec3 dDir_dy = dn_from_dv(vn, vlen, dv_dy) * view.cam;
	...
	float t_hit = length(Pos - oPos);
	vec3 dPos_dx = t_hit * dDir_dx;
	vec3 dPos_dy = t_hit * dDir_dy;
	...
	vec3 dtuv_dx = dPos_dx * vec3(3.0f, 3.0f, 1.5f);
	vec3 dtuv_dy = dPos_dy * vec3(3.0f, 3.0f, 1.5f);
	vec2 uv_yz = tuv.yz();
	vec2 uv_xz = tuv.xz();
	vec2 uv_xy = tuv.xy();
	vec2 dYZdx = dtuv_dx.yz();
	vec2 dYZdy = dtuv_dy.yz();
	vec2 dXZdx = dtuv_dx.xz();
	vec2 dXZdy = dtuv_dy.xz();
	vec2 dXYdx = dtuv_dx.xy();
	vec2 dXYdy = dtuv_dy.xy();

	float dd;
	if (g_use_nmap) {
		float nms = 0.19f;
		vec3 nmx = nmap(uv_yz, ch0, nms, dYZdx, dYZdy) + nmap(-uv_yz, ch0, nms, -dYZdx, -dYZdy);
		vec3 nmy = nmap(uv_xz, ch1, nms, dXZdx, dXZdy) + nmap(-uv_xz, ch1, nms, -dXZdx, -dXZdy);
		vec3 nmz = nmap(uv_xy, ch2, nms, dXYdx, dXYdy) + nmap(-uv_xy, ch2, nms, -dXYdx, -dXYdy);
	...
	vec4 col;
	if (g_use_albedo) {
		vec4 tx = texGrad(ch0, uv_yz, dYZdx, dYZdy) + texGrad(ch0, -uv_yz, -dYZdx, -dYZdy);
		vec4 ty = texGrad(ch1, uv_xz, dXZdx, dXZdy) + texGrad(ch1, -uv_xz, -dXZdx, -dXZdy);
		vec4 tz = texGrad(ch2, uv_xy, dXYdx, dXYdy) + texGrad(ch2, -uv_xy, -dXYdx, -dXYdy);
...
nk_bool do_user_gui(struct nk_context* ctx, nk_bool is_playing)
{
	...
	nk_label(ctx, "The Cave (mipmapped)", NK_TEXT_CENTERED);
...
void mainImage(vec4* fragColor, vec2 fragCoord)
{
	...
	vec2 uv = 2.f*fragCoord/r2-1.f;
	float aspect = iResolution.z;

	vec3 v = vec3(uv*vec2(aspect,1.0f),1.0f);
	float vlen = length(v);
	vec3 vn = v / vlen;
	vec3 Dir = vn * cam;

	vec3 dv_dx = vec3(aspect * 2.f / r2.x, 0.f, 0.f);
	vec3 dv_dy = vec3(0.f, 2.f / r2.y, 0.f);
	vec3 dDir_dx = dn_from_dv(vn, vlen, dv_dx) * cam;
	vec3 dDir_dy = dn_from_dv(vn, vlen, dv_dy) * cam;
	...
	if (g_use_nmap) {
		float nms=0.19f;
		vec3 nmx = nmap(uv_yz, iChannel0, nms, dYZdx, dYZdy) + nmap(-uv_yz, iChannel0, nms, -dYZdx, -dYZdy);
		vec3 nmy = nmap(uv_xz, iChannel1, nms, dXZdx, dXZdy) + nmap(-uv_xz, iChannel1, nms, -dXZdx, -dXZdy);
		vec3 nmz = nmap(uv_xy, iChannel2, nms, dXYdx, dXYdy) + nmap(-uv_xy, iChannel2, nms, -dXYdx, -dXYdy);
		vec3 nn=normalize(nmx*tpn.x+nmy*tpn.y+nmz*tpn.z);
		dd=max(0.0f ,dot(nn,normalize(ld*mat3(vec3(1,0,0),vec3(0,0,1),n))));
	} else {
		dd=max(0.0f ,dot(n,normalize(ld)));
	...
	vec4 col;
	if (g_use_albedo) {
		vec4 tx = texGrad(iChannel0, uv_yz, dYZdx, dYZdy) + texGrad(iChannel0, -uv_yz, -dYZdx, -dYZdy);
		vec4 ty = texGrad(iChannel1, uv_xz, dXZdx, dXZdy) + texGrad(iChannel1, -uv_xz, -dXZdx, -dXZdy);
		vec4 tz = texGrad(iChannel2, uv_xy, dXYdx, dXYdy) + texGrad(iChannel2, -uv_xy, -dXYdx, -dXYdy);
		col = tx*tpn.x+ty*tpn.y+tz*tpn.z;
	} else {
		col = vec4(1.2f);
	}
